...
export function MobileNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-[100] flex h-16 items-center justify-around overflow-x-hidden border-t border-border bg-background/95 px-2 backdrop-blur-xl md:hidden">
      {allItems.map((item) => (
        <NavLink
          key={item.url}
          to={item.url}
