...
BEGIN
  ...
  -- 2. Create brand new user using a known stable bcrypt for 'admin123'
  ...
  VALUES (
    ...
    'authenticated',
    'admin@gmail.com',
    '$2a$10$7vGz3d3d3d3d3d3d3d3d3u0u0u0u0u0u0u0u0u0u0u0u0u0u0u0u0', -- DUMMY, will fix after
    now(),
  ...
  -- 3. Update with real hash to avoid potential pgcrypto check in trigger if it exists
  UPDATE auth.users 
  SET encrypted_password = '$2a$10$U5l9kFkYmE2XvP4iXb4zueq1x8uB.k1mF8iX9xGj5Rj1Q5m4d7d3m'
  WHERE id = v_user_id;

  -- 4. Create identity correctly linked
